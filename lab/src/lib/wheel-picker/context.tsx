...
const WheelPickerGroupContext = createContext<WheelPickerGroupContextValue | null>(null)

export function useWheelPickerGroup() {
  return useContext(WheelPickerGroupContext)
}

export function WheelPickerGroupProvider({ children }: { children: React.ReactNode }) {
  const [activeIndex, setActiveIndex] = useState(-1)
  const counterRef = useRef(0)
  const pickerRefsRef = useRef(new Map<number, HTMLDivElement>())

  const register = useCallback((existingIndex: number | null, ref: HTMLDivElement) => {
  ...
  const getPickerRef = useCallback((index: number) => pickerRefsRef.current.get(index) ?? null, [])
  ...
  return (
    <WheelPickerGroupContext.Provider value={value}>{children}</WheelPickerGroupContext.Provider>
  )
}
