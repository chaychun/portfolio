...
const getOptionTextValue = <T extends WheelPickerValue>(option: WheelPickerOption<T>): string => {
  return (
    option.textValue ?? (typeof option.label === "string" ? option.label : String(option.value))
  )
}
...
function WheelPicker<T extends WheelPickerValue>({
  ...
  const renderWheelItems = useMemo(() => {
    const renderItem = (item: WheelPickerOption<T>, index: number, angle: number) => (
      <li
        key={index}
